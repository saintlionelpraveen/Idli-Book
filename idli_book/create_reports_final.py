"""
Create all 14 IB Reports - Production Ready Solution
Run with: bench execute idli_book.create_reports_final.create_all_reports
"""
...
def create_all_reports():
    """Create or update all 14 IB production reports using direct SQL"""
    ...
    for config in reports_config:
        try:
            ...
            if frappe.db.exists("Report", config["name"]):
                frappe.db.sql("DELETE FROM `tabReport` WHERE name = %s", config["name"])
                print(f"🗑️  Deleted existing: {config['name']}")
            
            # Insert directly into database (bypasses document validation issues)
            frappe.db.sql("""
                INSERT INTO `tabReport` 
                (name, creation, modified, modified_by, owner, docstatus, idx,
                 report_name, ref_doctype, report_type, module, is_standard,
                 query, filters, add_total_row, disabled, prepared_report)
                VALUES 
            ...
            """, (
                ...
                config["ref_doctype"],  # ref_doctype
                "Query Report",  # report_type
                "Idli Book",  # module
                "Yes",  # is_standard
                config["query"],  # query
                json.dumps(config["filters"]),  # filters (as JSON string)
                1,  # add_total_row
                0,  # disabled
                0  # prepared_report
            ))
    ...
    if created == 14:
        ...
        print("\n⚠️  Important: Run 'bench restart' to load the reports!")
